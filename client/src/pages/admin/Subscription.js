  ...
  const getAllSubscription = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/v1/subscribe/get/subscription`
      );
      setSubscriptions(data?.subscribe);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const { data } = await axios.delete(
        `${process.env.REACT_APP_API_URL}/api/v1/subscribe/delete/subscription/${id}`
      );
      if (data?.success) {
        setDeleting(false);
